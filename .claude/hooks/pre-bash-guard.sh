...
if echo "$COMMAND" | grep -qE 'rm\s+-[a-zA-Z]*r[a-zA-Z]*f|rm\s+-[a-zA-Z]*f[a-zA-Z]*r'; then
  # Allow recursive-force rm on safe targets (node_modules, dist, coverage, .vite,
  # /tmp). Matches any flag spelling the detector matches (-rf, -fr, -rfv, ...) and
  # allows ./-prefixed or relative-path-prefixed targets (e.g. ./dist, packages/app/dist).
  # Absolute paths stay blocked except /tmp.
  if echo "$COMMAND" | grep -qE 'rm\s+-[a-zA-Z]+\s+(\./)?([^ /][^ ]*/)?(node_modules|dist|coverage|\.vite|\.turbo)/?(\s|$)|rm\s+-[a-zA-Z]+\s+/tmp(/[^ ]*)?(\s|$)'; then
